...
class Data:
    ...
    def __repr__(self):
        return "ticks={}, cpu={}, vmem={}, KBout={}, KBin={}, conn={}".format(
            self.ticks, self.Y1, self.Y2, self.Y3, self.Y4, self.Y5
        )
    ...
    return html.Div(
        children=[
            ...
            html.Div(
                [
                    html.H4(
                        children="This is a python web application created using Dash python web framework and deployed on Heroku Dyno (container)"
                    ),
                    html.P(
            ...
            html.Table(
                className="table",
                children=[html.Tr([html.Th("Attribute"), html.Th("Value")])]
                + [
                    ...
                    html.Tr(
                        [
                            html.Td("CPU Clock"),
                            html.Td("{} MHz".format(int(ps.cpu_freq().current))),
                        ]
                    ),
                    html.Tr(
                        [html.Td("RAM"), html.Td("{} GB".format(ps.virtual_memory().total >> 30))]
                    ),
            ...
            html.Div(
                ...
                children=[
                    dcc.Graph(id="my_graph", animate=True),
                    dcc.Interval(id="my_graph_update", interval=2000),  # ms
                ],
            ),
            html.Div(
                className="border border-primary",
                children=[
                    dcc.Graph(id="my_graph2", animate=True),
                    dcc.Interval(id="my_graph_update2", interval=2000),  # ms
                ],
            ),
            html.Div(
                className="border border-primary",
                children=[
                    dcc.Graph(id="my_graph3", animate=True),
                    dcc.Interval(id="my_graph_update3", interval=2000),  # ms
                ],
            ),
            html.Div(
                [
                    html.Footer(
                        children=[
                            html.A("Tarun Sharma", href="https://www.linkedin.com/in/tarun27sh/")
                        ]
                    ),
...
app.scripts.append_script(
    {"external_url": "https://stackpath.bootstrapcdn.com/bootstrap/4.1.3/js/bootstrap.min.js"}
)
app.css.append_css(
...
@app.callback(Output("my_graph", "figure"), events=[Event("my_graph_update", "interval")])
def my_graph_update():
    data = Data()
    print("Update called      - ")
    get_lsof()
    ...
    scatter_data1 = go.Scatter(x=x_data, y=y_cpu, name="CPU", mode="lines+markers", fill="tozeroy")
    scatter_data2 = go.Scatter(
        x=x_data, y=y_vmem, name="Virtual Memory", mode="lines+markers", fill="tonexty"
    )
    ...
    return {
        ...
        "layout": go.Layout(
            ...
            xaxis={"title": "Units: Seconds", "range": [min(x_data), max(x_data)]},
            yaxis={"title": "%age", "range": [0, 100]},
        ),
    }


@app.callback(Output("my_graph2", "figure"), events=[Event("my_graph_update2", "interval")])
def my_graph_update2():
    ...
    scatter_data3 = go.Scatter(
        x=x_data, y=y_sent, name="KB Sent", mode="lines+markers", fill="tozeroy"
    )
    scatter_data4 = go.Scatter(
        x=x_data, y=y_recv, name="KB Recv", mode="lines+markers", fill="tonexty"
    )
    ...
    return {
        ...
        "layout": go.Layout(
            ...
            xaxis={"title": "Units: Seconds", "range": [min(x_data), max(x_data)]},
            yaxis={"title": "KB", "range": [min(y_sent + y_recv), max(y_sent + y_recv)]},
        ),
    }


@app.callback(Output("my_graph3", "figure"), events=[Event("my_graph_update3", "interval")])
def my_graph_update3():
    ...
    scatter_data = go.Scatter(
        x=x_data, y=y_conn, name="No of connections", mode="lines+markers", fill="tonexty"
    )
    return {
        "data": [scatter_data],
        "layout": go.Layout(
            ...
            xaxis={"title": "Units: Seconds", "range": [min(x_data), max(x_data)]},
            yaxis={"title": "#connections", "range": [0, max(y_conn)]},
        ),
    }
