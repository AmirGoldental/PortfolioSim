...
anes = pd.read_csv(
    "https://github.com/jkropko/DS-6001/raw/master/localdata/anes_pilot2019_clean.csv",
    low_memory=False,
)

anes_state = pd.read_csv(
    "https://github.com/jkropko/DS-6001/raw/master/localdata/anes_pilot_2019.csv"
)
anes_state = anes_state[["caseid", "inputstate"]]
anes_state["state"] = anes_state["inputstate"].map(
    {
        1: "Alabama",
        2: "Alaska",
        60: "American Samoa",
...
anes_state["state_abb"] = anes_state["inputstate"].map(
    {
        1: "AL",
        2: "AK",
        60: "AS",
...
### Table
anes_display = anes.groupby("vote").agg({"vote": "size", "age": "mean"})
anes_display["percent"] = 100 * anes_display.vote / sum(anes_display.vote)

anes_display = pd.merge(
    anes_display,
    100 * pd.crosstab(anes.vote, anes.liveurban, normalize="index"),
    left_index=True,
    right_index=True,
)
anes_display = anes_display[["vote", "percent", "age", "City", "Rural", "Suburb", "Town"]]
anes_display = anes_display.rename(
    {
        "vote": "Votes",
        "age": "Avg. age",
        "percent": "Percent",
...
### Barplot
colpercent = round(
    100 * pd.crosstab(anes.vote, anes.partyID, normalize="columns"), 2
).reset_index()
colpercent = pd.melt(
    colpercent, id_vars="vote", value_vars=["Democrat", "Republican", "Independent"]
)
...
rowpercent = pd.melt(
    rowpercent, id_vars="vote", value_vars=["Democrat", "Republican", "Independent"]
)
...
ftb = (
    pd.crosstab(anes.vote, anes.partyID, values=anes.ftbiden, aggfunc="mean")
    .round(2)
    .reset_index()
)
...
ftt = (
    pd.crosstab(anes.vote, anes.partyID, values=anes.fttrump, aggfunc="mean")
    .round(2)
    .reset_index()
)
...
anes_line = (
    anes.query("age <= 85").groupby("age").agg({"ftbiden": ["mean", "median", q25, q75, iqr]})
)
...
anes_line2 = (
    anes.query("age <= 85").groupby("age").agg({"fttrump": ["mean", "median", q25, q75, iqr]})
)
...
### Violin plot
anes_cand = pd.melt(
    anes, id_vars=["caseid"], value_vars=["ftbiden", "fttrump", "ftobama", "ftsanders"]
)
anes_cand = anes_cand.rename({"variable": "candidate", "value": "thermometer"}, axis=1)
anes_cand["candidate"] = anes_cand["candidate"].map(
    {
        "ftbiden": "Joe Biden",
...
anes_state["result"] = pd.cut(
    anes_state.difference, [-100, -0.00001, 0, 100], labels=["biden", "tie", "trump"]
)
anes_state = pd.merge(
    anes_state, anes.groupby(["state", "state_abb"]).size().reset_index(), on="state_abb"
)
...
fig_map = px.choropleth(
    ...
    locations="state_abb",
    hover_name="state",
    hover_data=["Donald Trump", "Joe Biden", "difference", "voters"],
    locationmode="USA-states",
    color="result",
    scope="usa",
    color_discrete_map={"biden": "blue", "tie": "purple", "trump": "red"},
)
...
app.layout = html.Div(
    [
        ...
        html.Div(
            [html.H2("Vote Choice By State"), dcc.Graph(figure=fig_map)],
            style={"width": "48%", "float": "left"},
        ),
        html.Div(
            [html.H2("Support by Age Group"), dcc.Graph(figure=fig_line)],
        ...
        html.Div(
            [
                ...
                html.H3("colors"),
                dcc.Dropdown(id="color", options=[{"label": i, "value": i} for i in cat_columns]),
            ],
            style={"width": "25%", "float": "left"},
        ),
        html.Div([dcc.Graph(id="graph")], style={"width": "70%", "float": "right"}),
    ]
)
